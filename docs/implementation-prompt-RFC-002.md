# Implementation Prompt for RFC-002: Application Shell, Dark Theme, and Sidebar Navigation
...
## Context
This implementation covers RFC-002, which focuses on creating the visual and navigational skeleton of the entire application: root Next.js layout with enforced dark theme, collapsible sidebar with team switcher, routing structure, and first-run empty state. Please refer to the following documents:
...
## Scope Limitation
Please only implement the features specified in @RFC-002.md. If you identify dependencies on features from other RFCs, note them but do not implement them unless explicitly instructed.
