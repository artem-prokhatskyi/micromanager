# Implementation Prompt for RFC-005: Sprint Management and Capacity Dashboard
...
## Context
This implementation covers RFC-005, which focuses on sprint creation via Jira lookup, the sprint dashboard header, capacity table with planned and overdue columns, team capacity totals, sprint selector dropdown, and per-sprint focus factor override. This RFC covers everything on the sprint dashboard except issue tables (RFC-007) and the calendar (RFC-006). Please refer to the following documents:
...
## Scope Limitation
Please only implement the features specified in @RFC-005.md. If you identify dependencies on features from other RFCs, note them but do not implement them unless explicitly instructed.
