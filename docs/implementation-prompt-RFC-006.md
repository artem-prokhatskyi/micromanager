# Implementation Prompt for RFC-006: Calendar and Non-Working Days
...
## Context
This implementation covers RFC-006, which focuses on the calendar tab with 3-month view, sprint band overlays, non-working day indicators with tooltips, the add/edit/delete popup, and the member filter bar. Non-working day records created here directly affect capacity calculations implemented in RFC-005. Please refer to the following documents:
...
## Scope Limitation
Please only implement the features specified in @RFC-006.md. If you identify dependencies on features from other RFCs, note them but do not implement them unless explicitly instructed.
